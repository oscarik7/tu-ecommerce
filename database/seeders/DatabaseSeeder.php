                                    ...
                                    <div class="space-y-1">
                                        @foreach($product->variants as $variant)
                                            <button wire:click="addToCart({{ $variant->id }})"
                                                class="w-full bg-purple-600 hover:bg-purple-700 text-white rounded-lg px-3 py-2 text-xs font-bold transition-all flex justify-between items-center"
                ...
                @else
                    <div class="space-y-3">
                        ...
                        @if(strlen($customerSearch) >= 2 && $customers->count() > 0)
                            <div class="bg-gray-50 rounded-lg p-2 max-h-40 overflow-y-auto">
                                @foreach($customers as $customer)
                                    <button wire:click="selectCustomer({{ $customer->id }})"
                                        class="w-full text-left px-3 py-2 hover:bg-purple-100 rounded-lg transition-all">
    ...
    @if($showPaymentModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div class="bg-white rounded-2xl max-w-md w-full p-6" @click.stop>
                ...
                <div class="flex gap-3">
                    <button type="button" wire:click="showPaymentModal = false" 
