<div>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-wrap gap-1">
                                    @forelse($product->variants->where('is_active', true) as $variant)
                                        <span class="inline-flex items-center text-xs font-medium text-purple-700 bg-purple-100 px-2 py-1 rounded-full">
                                            {{ $variant->volume }}ml
                                            <span class="ml-1 text-purple-500">({{ $variant->stock }})</span>
                                        </span>
                                    @empty
                                        <span class="text-xs text-gray-400 italic">Sin variantes activas</span>
                                    @endforelse
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($product->variants->count() > 0)
                                    <div class="text-sm font-bold text-gray-900">
                                        ...
                                        @endif
                                        <span class="text-xs text-gray-600 font-normal">Gs</span>
                                    </div>
                                @else
    ...
    <div class="flex items-center gap-3">
        ...
        @if($hasSales || $inCarts)
            {{-- Si tiene ventas o está en carritos, solo permitir desactivar --}}
            @if($product->is_active)
                <button 
                    wire:click="deactivateProduct({{ $product->id }})"
                    wire:confirm="Este producto tiene {{ $hasSales ? 'ventas asociadas' : 'items en carritos de clientes' }}. ¿Deseas desactivarlo? Los clientes ya no podrán verlo ni comprarlo."
            ...
            @endif
        @else
            {{-- Si no tiene ventas ni carritos, permitir eliminar --}}
            <button 
                        ...
                        <div class="bg-blue-50 rounded-lg p-5 space-y-4">
                            ...
                            <div>
                                ...
                                @error('image') 
                                    <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> 
                                @enderror
                                <p class="text-xs text-gray-500 mt-1">Formatos: JPG, PNG, WebP. Máximo 5MB. Se optimizará automáticamente.</p>
                            </div>

                            <div class="flex items-start gap-4">
                                @if ($image)
                                    <div class="flex-1">
                                        <p class="text-sm font-medium text-gray-700 mb-2">Vista previa de la nueva imagen:</p>
                                ...
                                @foreach($variants as $index => $variant)
                                    <div class="bg-white rounded-lg p-4 border-2 {{ !empty($variant['price']) && $variant['price'] > 0 ? 'border-green-300' : 'border-gray-200' }} shadow-sm hover:shadow-md transition">
                                        ...
                                        <div class="space-y-3">
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700 mb-1">
                                                    Precio (Gs) <span class="text-red-500">*</span>
                                                </label>
                                                <div class="relative">
                                        ...
                                        @if(!empty($variant['price']) && $variant['price'] > 0)
                                            <div class="mt-3 pt-3 border-t border-gray-200">
                                                <div class="flex justify-between text-xs">
                                                    <span class="text-gray-600">Precio configurado:</span>
